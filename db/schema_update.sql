# modyfikacja obecnej tabeli Field
ALTER TABLE `FIELD` 
DROP FOREIGN KEY `FK_FIELD_FIELDFORMAT`,
DROP FOREIGN KEY `FK_FIELD_FIELDCONTEXT`;
ALTER TABLE `FIELD` 
DROP COLUMN `FIELDFORMATID`,
DROP COLUMN `FIELDCONTEXTID`,
ADD COLUMN `MINVALUE` INT UNSIGNED NULL DEFAULT NULL AFTER `ID`,
ADD COLUMN `MAXVALUE` INT UNSIGNED NULL DEFAULT NULL AFTER `MINVALUE`,
ADD COLUMN `ISHIDDEN` TINYINT(1) NULL DEFAULT NULL AFTER `MAXVALUE`,
ADD COLUMN `TYPE` VARCHAR(25) NOT NULL DEFAULT 'TEXT' AFTER `TYPE`,
DROP INDEX `FK_FIELD_FIELDFORMAT`,
DROP INDEX `FK_FIELD_FIELDCONTEXT` ;

# usunięcie niepotrzebnych tabel
DROP TABLE `FIELDCONTEXT`;
DROP TABLE `FIELDFORMAT`;

# stworzenie tabeli przechowujacej powiazania
# miedzy customowymi polami a projektem
CREATE TABLE `PROJECTFIELDS` (
  `PROJECTID` INT(10) UNSIGNED NOT NULL,
  `FIELDID` INT(10) UNSIGNED NOT NULL,
  PRIMARY KEY (`PROJECTID`, `FIELDID`),
  INDEX `FK_PROJECTFIELDS_PROJECT` (`PROJECTID` ASC),
  INDEX `FK_PROJECTFIELDS_FIELD` (`FIELDID` ASC),
  CONSTRAINT `FK_PROJECTFIELDS_FIELD`
    FOREIGN KEY (`FIELDID`)
    REFERENCES `FIELD` (`ID`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `FK_PROJECTFIELDS_PROJECT`
    FOREIGN KEY (`PROJECTID`)
    REFERENCES `PROJECT` (`ID`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT);

# stworzenie tabeli z wartosciami fieldow poszczegolnych rzeczy
CREATE TABLE `FIELDSVALUES` (
  `ID` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `FIELDID` INT(10) UNSIGNED NOT NULL,
  `VALUE` VARCHAR(255) NULL,
  `CONTEXT` VARCHAR(45) NOT NULL,
  `CONTEXTID` INT(10) UNSIGNED NOT NULL,
  PRIMARY KEY (`ID`),
  INDEX `FK_FIELDVALUES_FIELDID` (`FIELDID` ASC),
  CONSTRAINT `FK_FIELDVALUES_FIELDID`
    FOREIGN KEY (`FIELDID`)
    REFERENCES `FIELD` (`ID`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT);


# stworzenie column dla standard fields
ALTER TABLE  `tracker` ADD  `ASSIGNED` TINYINT( 1 ) NULL DEFAULT NULL AFTER  `ID` ,
ADD  `CATEGORY` TINYINT( 1 ) NULL DEFAULT NULL AFTER  `ASSIGNED` ,
ADD  `FIXEDVERSION` TINYINT( 1 ) NULL DEFAULT NULL AFTER  `CATEGORY` ,
ADD  `PARENTISSUE` TINYINT( 1 ) NULL DEFAULT NULL AFTER  `FIXEDVERSION` ,
ADD  `STARTDATE` TINYINT( 1 ) NULL DEFAULT NULL AFTER  `PARENTISSUE` ,
ADD  `DUEDATE` TINYINT( 1 ) NULL DEFAULT NULL AFTER  `STARTDATE` ,
ADD  `ESTIMATEDHOURS` TINYINT( 1 ) NULL DEFAULT NULL AFTER  `DUEDATE` ,
ADD  `DONERADIO` TINYINT( 1 ) NULL DEFAULT NULL AFTER  `ESTIMATEDHOURS` ;
